...
if(isset($_POST['sensorID'])){
    ...
    while ($startDate <= $endDate) {
        ...
        for ($i = 1; $i <= 5; $i++) {
            ...
            DBUtils::writeSensorMeasurement(rand(5,60), "$formattedDate $formattedTime", $_POST["sensorID"]);
        }

        // Move to the next day
        $startDate->modify('+1 day');
    }
    $success = DBUtils::writeSensorMeasurement(rand(5,60), date('Y-m-d H:i:s'), $_POST["sensorID"]);
    echo "seeded";
}
?>